...
Respond ONLY with the clean markdown text representing the article. Do not wrap the output in a ```markdown code block or JSON."""
        ...
        while current_key_idx < len(GEMINI_KEY_POOL):
            active_key = GEMINI_KEY_POOL[current_key_idx]
            key_preview = f"...{active_key[-6:]}" if len(active_key) > 6 else "Key"
            log_info(f"Using Gemini API Key #{current_key_idx + 1} ({key_preview}) to format Markdown story.")
            ...
            try:
                ...
                log_warn("google-generativeai client library missing. Skipping AI formatting.")
                break
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    log_warn(f"Retry {attempt}/3 failed using Gemini Key #{current_key_idx + 1}: {e}")
                    time.sleep(1.5)
            
            # Since 3 retries failed for this key, rotate immediately to the next API key
            log_error(f"Gemini API Key #{current_key_idx + 1} exhausted or billing-blocked. Rotating keys...")
            current_key_idx += 1
...
def run_crawler():
    ...
    # Process only the indices requiring scraper updates
    for count, idx in enumerate(new_indices):
        item = news_items[idx]
        ref_num = count + 1
        
        print(f"\n[{ref_num}/{len(new_indices)}] Scraping: {Col.BOLD}{item['title']}{Col.RESET}")
        print(f"       Source: {item['source']}  |  Author: {item['author']}")
        ...
        if scraped_success:
            log_success(f"Scraped {len(paragraphs)} paragraphs, {len(body_images)} inline body images, and categories: {categories}")
            scraped_count += 1
        else:
            log_warn("Web crawl did not retrieve body text paragraphs. Setting up description summary as fallback.")
        ...
        if db:
            try:
                ...
                log_success(f"Article successfully saved to Firestore path: /football_articles/{doc_safe_title}")
                saved_count += 1
            except Exception as e:
                log_error(f"Failed to submit parsed article payload to Firestore: {e}")
        else:
            # Print elegant CLI preview if database credential is not configured
            print(f"       {Col.YELLOW}--> Firestore offline. Article Metadata Preview (Truncated):{Col.RESET}")
            preview_subset = {
            ...
            print(json.dumps(preview_subset, indent=7))
            
        # Courteous pacing to be respectful to scraped webservers
        time.sleep(1)
